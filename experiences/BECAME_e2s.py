...
CFG = {
    ...
    # epoch 设置 debug
    # 'task1_epochs':        1,   # Task1 总训练轮数
    # 'task2_stage1_epochs': 1, # Task2 稳定容器阶段（SVD 投影）
    # 'task2_stage2_epochs': 1, # Task2 可塑性 + 蒸馏 + 融合阶段
    ...
    'save_path': '/root/autodl-fs/became/event2scene'
}
...
# =============================================================
# 2. 数据集：包一层把 task_name 带上（与原仓库 AppendName 作用一致）
# =============================================================
...
class TwoTaskTrainer:
    def __init__(self, cfg):
        ...
        self.pre_model = None                    # Task2 期间用，保存 Task1 结束时的模型
        self.m_model   = None                    # Task2 阶段1 结束时的 state_dict

        # EWC：在 BN 参数上施加
        self.reg_params = {n: p for n, p in self.model.named_parameters() if 'bn' in n}
        self.reg_terms  = {}                     # 包含 importance 与 task_param

        # Fisher（用于任务合并）——忽略 last 层
        ...
        self.task_count = 0                      # 已完成的任务数
        self.is_distill = False                  # 是否启用蒸馏
    ...
    def _build_optimizer(self, task_idx):
        ...
        # 只把 Linear/Conv2d 的 weight 纳入 SVD 组
        ...
        # 剩下的（LN、bias、Mamba 的其它 param 等）都归到 other_params
    ...
    # ---------- 一个 epoch ----------
    def _train_epoch(self, loader, task_name):
        self.model.train()
        tot, n = 0.0, 0
    ...
    #         self._update_cov(torch.mean(fea_in[0], 0, True), module.weight)
    ...
    # @torch.no_grad()
    # def build_svd_transforms(self, loader):
    #     self.fea_in = defaultdict(dict)
    #     modules = [m for n, m in self.model.named_modules()
    #                if hasattr(m, 'weight') and not re.match(r'^detector', n)]
    #     self._hook_handles = [m.register_forward_hook(self._cov_hook) for m in modules]
    #     for _, features, targets_list, _, _, _, task_name in loader:
    #         self.model.forward(features.to(self.device), task_name=task_name)
    ...
    def build_svd_transforms(self, loader, task_name):
        ...
        self._hook_handles = [m.register_forward_hook(self._cov_hook) for m in modules]
        for _, features, _, _, _, _ in loader:
            self.model.forward(features.to(self.device), task_name=task_name)
        # svd_param_ids = {id(p)
        #          for g in self.optimizer.param_groups if g.get('svd', False)
        #          for p in g['params']}
        # recorded_ids = {id(k) for k, v in self.fea_in.items() if torch.is_tensor(v)}
        # missing = svd_param_ids - recorded_ids
        # assert not missing, f'[SVD] 有 {len(missing)} 个 SVD 参数没拿到协方差，检查 hook 过滤器'
        svd_id2name = {}
        ...
        # print(f'[SVD] 缺失协方差的 {len(missing_names)} 个参数：{missing_names}')
    ...
    def train_task1(self, train_loader, val_loader):
        ...
        self.optimizer.switch = True   # 首个任务无子空间投影（transforms 为空），switch 值不影响
        for ep in tqdm(range(self.cfg['task1_epochs']), desc='Task1'):
            loss = self._train_epoch(train_loader, task_name)
            self.validate(val_loader, task_name, ep=ep)
            self.save_checkpoint(ep, task_name, stage='phase1', is_best=False)
        # print(f'Task1 done. val acc = {acc1:.4f}')

        # 为 Task2 做准备：换新的分类头优化器 → 计算 SVD 子空间 → 记录 EWC / Fisher
        self.task_count = 1
    ...
    def train_task2(self, train_loader, val_loader, val_loader_task1=None):
        ...
        if self.cfg['task2_stage2_epochs'] > 0:
            ...
            # ---------- Fisher 加权融合：m_model(稳定) ⊕ 当前模型(可塑) ----------
        ...
        # 收尾：更新 Fisher（如果将来要训练 Task3 可用）
        self.update_fisher(train_loader, name2)

        # acc2 = self.validate(val_loader, name2)
        self.validate(val_loader, name2)
        # results = {'task2': acc2}
        if val_loader_task1 is not None:
            # results['task1'] = self.validate(val_loader_task1, name1)
            self.validate(val_loader_task1, name1)
            # print(f'[After Task2] acc1={results["task1"]:.4f}  acc2={acc2:.4f}  '
            #       f'avg={np.mean(list(results.values())):.4f}')
        # else:
        #     print(f'[After Task2] acc2={acc2:.4f}')
        # return results
