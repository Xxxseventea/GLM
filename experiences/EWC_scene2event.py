...
def main():
 ...
 # =============================任务A：scene======================
    for ep in range(CFG.EVENT_EPOCHS):
        for batch in tqdm(scene_train_loader, desc=f"[scene ep{ep}]"):
            ewc.forward_backward_update(batch, scene_loss_fn)
    ...
    print("\n========== Task B: Event (with EWC penalty) ==========")
    for ep in range(CFG.SCENE_EPOCHS):
        pbar = tqdm(event_train_loader, desc=f"[Event ep{ep}]")
        for batch in pbar:
            total, task, reg = ewc.forward_backward_update(batch, event_loss_fn)
            pbar.set_postfix(total=total, task=task, ewc=reg)

    results = eval_event(event_val_loader, ewc.model, CFG.DEVICE)
    evaluate_multi_thresholds(
        results,
        CFG.ANNOTATION_PATH,
        thresholds=None,  # 或者你想指定的阈值列表，如 [0.1,0.2,...]
        log_tag=f"event_eval_ep2",
    )
